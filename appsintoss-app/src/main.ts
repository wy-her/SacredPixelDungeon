...
function notifyAdComplete() {
    ...
    const tryNotify = () => {
        ...
        if (attempts < maxAttempts) {
            ...
            // 최후의 수단: postMessage 시도
            try {
                const gameFrame = document.getElementById('game-frame') as HTMLIFrameElement;
                if (gameFrame && gameFrame.contentWindow) {
                    gameFrame.contentWindow.postMessage({ type: 'adComplete' }, '*');
                    console.log('Interstitial ad: sent postMessage as fallback');
                }
            } catch (e) {
    ...
    const doShowAd = () => {
        showFullScreenAd({
            ...
            onEvent: (showEvent) => {
                ...
                // 광고가 표시되면 타임아웃 취소 (유저가 광고 보는 시간은 무제한)
                if (showEvent.type === 'show' || showEvent.type === 'impression') {
                    clearTimeout(timeout);
                    console.log('Interstitial ad: timeout cleared, ad is showing');
                }
                // 모든 종료 이벤트 처리
