...
public class ItemSlot extends Button {
	...
	// DEBUG: Override onClick to trace when it's called
	@Override
	protected void onClick() {
		DeviceCompat.log("ItemSlot", "onClick() called! item=" + (item != null ? item.name() : "null"));
		// Subclasses should override this
	}

	// DEBUG: Override onPointerDown to trace touch events
	@Override
	protected void onPointerDown() {
		DeviceCompat.log("ItemSlot", "onPointerDown() item=" + (item != null ? item.name() : "null") +
			" active=" + active + " visible=" + visible);
		super.onPointerDown();
	}

	// DEBUG: Override onPointerUp to trace touch events
	@Override
	protected void onPointerUp() {
		DeviceCompat.log("ItemSlot", "onPointerUp() item=" + (item != null ? item.name() : "null") +
			" clickReady=" + clickReady);
		super.onPointerUp();
	}

	// DEBUG: Method to dump hotArea state
	public void debugDumpState() {
		Camera c = hotArea.camera();
		DeviceCompat.log("ItemSlot", "=== ItemSlot Debug Dump ===");
		DeviceCompat.log("ItemSlot", "  item: " + (item != null ? item.name() : "null"));
		DeviceCompat.log("ItemSlot", "  active: " + active + ", visible: " + visible);
		DeviceCompat.log("ItemSlot", "  isActive(): " + isActive());
		DeviceCompat.log("ItemSlot", "  Component bounds: x=" + x + " y=" + y + " w=" + width + " h=" + height);
		DeviceCompat.log("ItemSlot", "  hotArea bounds: x=" + hotArea.x + " y=" + hotArea.y +
			" w=" + hotArea.width + " h=" + hotArea.height);
		DeviceCompat.log("ItemSlot", "  hotArea.active: " + hotArea.active + " hotArea.visible: " + hotArea.visible);
		DeviceCompat.log("ItemSlot", "  hotArea.blockLevel: " + hotArea.blockLevel);
		DeviceCompat.log("ItemSlot", "  camera: " + (c != null ? "exists" : "NULL!"));
		if (c != null) {
			DeviceCompat.log("ItemSlot", "    camera pos: x=" + c.x + " y=" + c.y);
			DeviceCompat.log("ItemSlot", "    camera size: w=" + c.width + " h=" + c.height);
			DeviceCompat.log("ItemSlot", "    camera zoom: " + c.zoom);
			DeviceCompat.log("ItemSlot", "    camera scroll: x=" + c.scroll.x + " y=" + c.scroll.y);
			DeviceCompat.log("ItemSlot", "    screenWidth: " + c.screenWidth() + " screenHeight: " + c.screenHeight());
		}
		DeviceCompat.log("ItemSlot", "  parent chain: " + getParentChain());
		DeviceCompat.log("ItemSlot", "=== End Debug Dump ===");
	}
